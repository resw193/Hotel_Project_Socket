...
public class FormChangeRoom extends JDialog {
    ...
    public FormChangeRoom(Window owner,
                          String oldRoomID,
    ...
    private void initUI() {
        ...
        lblBookingInfo = new JLabel("-");
        lblBookingInfo.setForeground(new Color(0xE9EEF6));

        JButton btnChoose = new JButton("Tiếp tục");
        JButton btnClose = new JButton("Đóng");

        root.add(label("Phòng cũ:"));
        root.add(value(oldRoomID));

        root.add(label("Phòng mới:"));
        root.add(cbxNewRoom, "growx");

        root.add(label("Booking hiện tại:"));
        root.add(lblBookingInfo, "growx");
    ...
    private void loadBookingInfo() {
        try {
            BaseResponse response = sendRequest(CommandType.GET_PENDING_BOOKING, new RoomIdRequestDTO(oldRoomID));
            if (!response.isSuccess()) {
                lblBookingInfo.setText(response.getMessage());
                return;
            }

            OdrInfoDTO odr = (OdrInfoDTO) response.getData();
            if (odr == null) {
                lblBookingInfo.setText("Không có booking chờ.");
            } else {
                lblBookingInfo.setText(
    ...
    private void openDetail() {
        ...
        try {
            ...
            OdrInfoDTO odr = (OdrInfoDTO) response.getData();
            if (odr == null) {
                JOptionPane.showMessageDialog(this, "Không có booking chờ hợp lệ.");
                return;
            }

            FormChangeRoomDetail detail = new FormChangeRoomDetail(
                    this,
                    oldRoomID,
                    newRoom,
                    odr,

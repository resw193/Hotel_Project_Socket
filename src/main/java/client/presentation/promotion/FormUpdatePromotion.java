...
public class FormUpdatePromotion extends JDialog {
    ...
    public FormUpdatePromotion(Window owner, String promotionID) {
        ...
        add(label("Số lượng:"), "gaptop 4, gapbottom 6");
        ...
        buttonPanel.add(btnUpdate, "skip, w 100!, h 36!");
    ...
    private void loadData() {
        try {
            BaseResponse response = sendRequest(CommandType.GET_PROMOTION_BY_ID, promotionID);
            ...
            PromotionDTO p = (PromotionDTO) response.getData();
            if (p == null) {
    ...
    private void updatePromotion() {
        try {
            BaseResponse getRes = sendRequest(CommandType.GET_PROMOTION_BY_ID, promotionID);
            if (!getRes.isSuccess()) {
                JOptionPane.showMessageDialog(this, getRes.getMessage());
                return;
            }

            PromotionDTO p = (PromotionDTO) getRes.getData();
            if (p == null) {
                JOptionPane.showMessageDialog(this, "Không tìm thấy khuyến mãi.");
                return;
            }
            ...
            int quantity = Integer.parseInt(txtQuantity.getText().replace(",", "").trim());
            ...
            BaseResponse updateRes = sendRequest(CommandType.UPDATE_PROMOTION, p);
            if (updateRes.isSuccess()) {
                JOptionPane.showMessageDialog(this, "Đã cập nhật khuyến mãi thành công");
                dispose();
            }
            else {
